...
exports['default'] = function (grunt) {
	...
	function processor(options, plugins) {
		...
		plugins.forEach(function (plugin) {
			if (plugin.required && scope === 0 || plugin.occurances === 'multiple' || plugin.occurances === 'single' && scope === 0) {
	...
	grunt.registerMultiTask('cordova_config', 'Generates cordova config.xml', function () {
		var plugins = ['base', 'author', 'content', 'access', 'allowIntent', 'allowNavigation', 'preference', 'feature', 'icon', 'splash', 'platform', 'engine', 'plugin', 'hook'];

		grunt.file.write(this.data.dest, (0, _js2xmlparser2['default'])('widget', processor(this.options(), plugins.map(function (plugin) {
			return require('./configs/' + plugin);
		}))));
